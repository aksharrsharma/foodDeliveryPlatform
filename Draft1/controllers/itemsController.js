...
exports.create = (req, res) =>{
    ...
    if(req.file){ // must have if for validation (no image then you cannot append req.file.filename)
        item.image = '/images/' + req.file.filename; // Store image path
    }
    ...
    .then(item=>{
        console.log(item);
        req.flash('success', 'You have successfully created an item!');
...
exports.show = (req, res) =>{
    ...
    .then(item=>{ //consume the promise from findById
        if(item){
            return res.render('./show', {item}); //item is the object in show.ejs
        }else{
